...
export class GroupDetailListControl implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
		...
		if (rowspan) {
			this.divDetailListWrapper.style.height = `${height}rem`;
		}
		else {
			this.divDetailListWrapper.style.height = "20rem";
		}

		this.container.appendChild(this.divDetailListWrapper);

		this._props.groupField = context.parameters.field.raw || '';
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void {
		...
		if (!context.parameters.listDataSet.loading) {
			if (context.parameters.listDataSet.paging != null && context.parameters.listDataSet.paging.hasNextPage == true) {
				...
				this._props.data =this._sortData(dataItems,context.parameters.field.raw || '');
	...
	private _sortData = <T,>(items: T[], columnKey: string, isSortedDescending?: boolean): T[] => {
	...
	private navigateToRecord(id: string, name: any): void {
		let record: any = this.context.parameters.listDataSet.records[id];//.getNamedReference();
		console.log(record);
		if (name) {
			let lookup = record.getValue(name);
			if (lookup) {
				record.entityName = lookup.etn;
				record.id = lookup.id.guid;
			}
		} else {
			record = record.getNamedReference();
		}
