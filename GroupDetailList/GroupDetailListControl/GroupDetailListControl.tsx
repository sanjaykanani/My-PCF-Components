...
export interface IGroupDetailListControlProps {
    ...
    groupField: any,
    triggerNavigate?: (id: string,name:any) => void;
...
export class GroupListControl extends React.Component<IGroupDetailListControlProps, IListControlState> {
    ...
    constructor(props: IGroupDetailListControlProps) {
        ...
        super(props);
       // props.data=this._sort(props.data, props.groupField);
        ...
        this._groupingColumn=props.groupField;
        this.state = {
            _items:   this._sort(props.data, props.groupField),
    ...
    private _buildColumns(listData: IListColumn[]): IColumn[] {
        ...
        for (var column of listData) {
            ...
            if (column.dataType && (column.dataType === "Lookup")) {
                iColumn.onRender = (item: any, index: number | undefined, column: IColumn | undefined) => (
                    <Link key={item.key} onClick={() => this.state._triggerNavigate(item.key,column?.key)}>{item[column?.fieldName!]}</Link>
                );
            }
    ...
    private _groupBy(items: any, fieldNames: any, isSortedDesending?: boolean) {
        let groups: any;
        // let fieldName="category1";
        let fieldName = this.props.groupField as string;
